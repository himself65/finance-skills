...
```
!`python3 -c 'import yfinance as yf; p=yf.Ticker("^GSPC").fast_info["lastPrice"]; print(f"PRICE={p:.0f}")' 2>/dev/null || echo "PRICE_UNAVAILABLE"`
```
...
| Python module available | `python3 -c 'import mod; print(mod.__version__)' 2>/dev/null` |
