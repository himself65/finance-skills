...
External API data — sentiment via Adanos, fundamental research and raw data via Funda AI (MCP + REST), Hormuz Strait monitoring, TradingView desktop app reading, Hyperliquid perp/spot reading, and read-only Interactive Brokers account reading.
...
| [ibkr-reader](plugins/data-providers/skills/ibkr-reader/) | Read-only [Interactive Brokers](https://www.interactivebrokers.com) account reader — account summary, balances, positions & cost basis, orders/trades, quotes, implied vol (underlying + per-contract), IV percentile, put/call OI, and price history via IBKR's official Agentic MCP (never places or cancels orders) |
